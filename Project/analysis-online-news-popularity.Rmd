...
news_ptest <- function(df) {
  ## Linear model considering all variables
  ...
  ##GLM model is_popular against all variables
...
popularity_shares_modeling <- function(df) {
  set.seed(123)
  train_ids <- createDataPartition(df$is_popular, p=0.7, list=FALSE)
  ...
  fit <- train(as.factor(is_popular) ~ global_sentiment_polarity +
                                kw_avg_avg +
                                category +
                                kw_min_min +
                                self_reference_avg_sharess +
  ...
  print(confusionMatrix(predict(fit, df_test), as.factor(df_test$is_popular)))
...
```{r}
popularity_shares_modeling(classification_df)
```
